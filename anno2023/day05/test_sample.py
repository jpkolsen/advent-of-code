...
from anno2023.day05.solution import (Map, RangeMap, convert_multiple_maps,
                                     get_seed_array, parse_input, part_one,
                                     part_two, part_two_numpy)

INPUT_FILE = Path(__file__).parent.joinpath('sample_input.txt')


def test_map_range():
    map_range = RangeMap(source_start=0, dest_start = 5, range_length=10)
...
def test_convert():
    map_range = RangeMap(source_start=5, dest_start=10, range_length=2)
...
def test_convert_negative():
    map_range = RangeMap(source_start=5, dest_start=2, range_length=10)
...
def test_map_convert(source: int, dest:int):
    _map = Map(range_maps=[RangeMap(0, 5, 2), RangeMap(10, 15, 2), RangeMap(12, 22, 10)])
...
def test_multiple_maps_convert(source: int, dest: int):
    maps = [Map(range_maps=[RangeMap(0, 5, 10)]), Map(range_maps=[RangeMap(10, 15, 2)])]
...
def test_convert_array():
    ...
    assert all(np.union1d(*range_map.convert_array(array)) == np.array([4, 7, 8, 9, 10, 11, 12]))

def test_convert_multiple_range_array():
    maps = Map(range_maps=[RangeMap(5, 10, 2), RangeMap(15, 20, 2)])
    array = np.array(range(4, 16))
    assert all(maps.convert_array(array) == np.array([4, 7, 8, 9, 10, 11, 12, 13, 14, 20]))
